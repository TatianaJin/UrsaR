...
class HdfsDiskReadTaskDesc : public DiskReadTaskDesc {
 public:
  HdfsDiskReadTaskDesc() = default;
  HdfsDiskReadTaskDesc(JobIdType job_id, TaskIdType task_id, ShardIdType shard_id, const std::vector<std::pair<std::string, size_t>>& block_desc,
                       double input_data_size)
      : DiskReadTaskDesc(job_id, task_id, shard_id), block_desc_(block_desc), input_data_size_(input_data_size) {
  ...
  base::BinStream& serialize(base::BinStream& bin_stream) const override {
    DiskReadTaskDesc::serialize(bin_stream);
  ...
  base::BinStream& deserialize(base::BinStream& bin_stream) override {
    DiskReadTaskDesc::deserialize(bin_stream);
    size_t size;
    bin_stream >> size;
