...
class has_serialize {
 private:
  template <typename U>
  static constexpr auto check(int) -> typename std::is_same<decltype(std::declval<U>().serialize(*(new BinStream))), BinStream&>::type;
...
class has_deserialize {
 private:
  template <typename U>
  static constexpr auto check(int) -> typename std::is_same<decltype(std::declval<U>().deserialize(*(new BinStream))), BinStream&>::type;
